...
2. Timpa (replace) `DATA_PENJUALAN.xlsx` dan/atau `MASTER_PROGRAM.xlsx` dengan file terbaru — **nama file dan nama sheet harus sama persis**:
   ...
   - `MASTER_PROGRAM.xlsx` → sheet `MASTER BARANG`, `NOMINAL WAJIB`, `PERIODE PROGRAM`.
3. Refresh halaman dashboard (kalau `npm run dev` sedang berjalan, cukup reload browser — tidak perlu restart server).
